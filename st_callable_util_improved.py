...
def get_streamlit_cb(
    ...
    class StreamHandler(BaseCallbackHandler):
        ...
        def _update_ui(self) -> None:
            ...
            # Add a script to control scrolling behavior if auto_scroll is enabled
            # Only scroll on certain updates based on scroll_frequency to reduce excessive scrolling
            should_scroll = (
                self.auto_scroll and 
                (self.update_count % self.scroll_frequency == 0 or 
                 len(self.text) < 500)  # Always scroll for short responses
            )
            
            if should_scroll:
                # This script will scroll to the bottom of the container only when new content is added
                # The smooth behavior reduces jitter
                scroll_script = """
                <script>
                    function scrollToBottom() {
                        // Use a more gentle scroll that doesn't always go all the way to the bottom
                        const currentPos = window.scrollY;
                        const targetPos = document.body.scrollHeight - window.innerHeight;
                        
                        // Only scroll if we're not already near the bottom
                        if (targetPos - currentPos > 200) {
                            window.scrollTo({
                                top: currentPos + Math.min(200, targetPos - currentPos),
                                behavior: 'smooth'
                            });
                        }
                    }
                ...
                """
                self.container.markdown(scroll_script, unsafe_allow_html=True)
        ...
        def on_llm_end(self, response, **kwargs) -> None:
            ...
            # Final scroll to ensure the complete response is visible
            if self.auto_scroll:
                # This script ensures we scroll all the way to the bottom at the end
                scroll_script = """
                <script>
